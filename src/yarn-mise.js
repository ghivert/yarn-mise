function selectToml(fs, resolve) {
  ...
  if (fs.existsSync(resolve('.mise.local.toml'))) return resolve('.mise.local.toml')
}
          ...
          for (const part of paths) {
            ...
            content.split('\n').reduce((isEnv, val) => {
              ...
              if (val.startsWith('_.file')) {
                const envs = readEnvFile(fs, path, tomlDir, val.split('=').map(v => v.trim())[1])
                Object.values(envs).forEach(([key, value]) => {
                  scriptEnv[key] = value
                })
              }
              if (val.includes('python.venv')) addVenvInPath(path, tomlDir, parse(val.split('=')[1]?.trim()), scriptEnv)
              if (isEnv) {
